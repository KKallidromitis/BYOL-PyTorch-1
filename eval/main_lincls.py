...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        if os.path.isfile(args.pretrained):
            ...
            assert set(msg.missing_keys) == {"fc.weight", "fc.bias"}, f"Got {msg}, expected only fc.weight, fc.bias to be missing"
    ...
    # Using BYOL optimizer (SGD w/ Nesterov, no weight decay) (Appendix C.1)
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.gpu is not None:
            images = images.cuda(args.gpu, non_blocking=True).contiguous() #Had to add contiguous, b/c .size() wasn't working
        target = target.cuda(args.gpu, non_blocking=True).contiguous()
...
def validate(val_loader, model, criterion, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            if args.gpu is not None:
                images = images.cuda(args.gpu, non_blocking=True).contiguous() 
            target = target.cuda(args.gpu, non_blocking=True).contiguous() 
