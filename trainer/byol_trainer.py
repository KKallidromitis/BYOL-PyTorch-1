...
class BYOLTrainer():
    ...
    def _forward_masked_byol_loss(self, preds, targets,masks,raw_mask,mask_target,Q,H,coords,raw_image):
        ...
        return  loss,zero,zero,inv_loss,mask_loss,mask_exists.float().sum(-1).mean().detach()
    ...
    def train_epoch(self, epoch, printer=print):
        ...
        while images is not None:
            ...
            tflag = time.time()
            #breakpoint()
            q, target_z,pinds, tinds,down_sampled_masks,raw_mask,mask_target,num_segs,applied_mask,Q,H,coords = self.model(view1, view2, self.mm, input_masks,view_raw,diff_transfrom,slic_labelmap,use_masknet,full_view_prior_mask,clustering_k)
            forward_time.update(time.time() - tflag)

            tflag = time.time()
            loss,eh_obj,eh_dist,inv_loss,mask_loss,num_indicator = self.forward_loss(q,target_z,down_sampled_masks,raw_mask,mask_target,Q,H,coords,view_raw)
