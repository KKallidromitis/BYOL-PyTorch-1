...
class DetconInfoNCECriterion(nn.Module):
    ...
    # SIMCLR CHANGE: Added num_rois param to handle num_masks we generate
    ...
    def make_same_obj(self,ind_0, ind_1, num_rois):
        b = ind_0.shape[0]
    ...
    def forward(self, target, pred, tind, pind, num_rois):        
        ...
        same_obj_aa = self.make_same_obj(pind1, tind1, num_rois)
        same_obj_ab = self.make_same_obj(pind1, tind2, num_rois)
        same_obj_ba = self.make_same_obj(pind2, tind1, num_rois)
        same_obj_bb = self.make_same_obj(pind2, tind2, num_rois)
        ...
        labels_0 = torch.reshape(labels_abaa, [self.batch_size, num_rois, -1])
        labels_1 = torch.reshape(labels_babb, [self.batch_size, num_rois, -1])
        ...
        obj_area_0 = torch.sum(self.make_same_obj(pind1, pind1, num_rois), axis=[2, 3])
        obj_area_1 = torch.sum(self.make_same_obj(pind2, pind2, num_rois), axis=[2, 3])
        ...
        logits_abaa = torch.reshape(logits_abaa, [self.batch_size, num_rois, -1])
        logits_babb = torch.reshape(logits_babb, [self.batch_size, num_rois, -1])
